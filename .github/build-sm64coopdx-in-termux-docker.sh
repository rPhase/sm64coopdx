...
if [ "$TARGET_ANDROID_ARCH" == "armeabi-v7a" ]; then
  ...
  do
    ...
    make TARGET_ARCH=$ARCHITECTURE TARGET_ANDROID_ARCH=$TARGET_ANDROID_ARCH TARGET_ANDROID=1 NOEXTRACT=1 -j32 && break
    attempt=$((attempt+1))
  ...
  make TARGET_ARCH=$ARCHITECTURE TARGET_ANDROID_ARCH=$TARGET_ANDROID_ARCH TARGET_ANDROID=1 NOEXTRACT=1 -j32
fi