...
void djui_panel_touch_controls_create(struct DjuiBase* caller) {
    ...
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        ...
        djui_slider_create(body, DLANG(CONTROLS, TOUCH_CONTROLS_OPACITY), &configTouchControlAlpha, 0, 255, NULL);
        djui_slider_create(body, DLANG(CONTROLS, TOUCH_CONTROLS_SCALE), &configAndroidBiggerButtons, 0, 2, NULL);

        djui_checkbox_create(body, DLANG(CONTROLS, TOUCH_AUTOHIDE), &configAutohideTouch, NULL);
        djui_checkbox_create(body, DLANG(CONTROLS, TOUCH_SLIDE_TOUCH), &configSlideTouch, NULL);
        djui_checkbox_create(body, DLANG(CONTROLS, TOUCH_IGNORE_PHANTOMS), &configPhantomTouch, NULL);

        struct DjuiButton* touchbindsButton = djui_button_create(body, DLANG(CONTROLS, TOUCH_EDIT_BINDS), DJUI_BUTTON_STYLE_NORMAL, djui_panel_shutdown_touchconfig);
